...
public class AlunoRepository : BaseRepository, IAlunoRepository
{
        ...
        int id, CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException("ERRO_OBTER_POR_ID",
                $"Erro ao obter aluno por ID {id}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorId
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException("ERRO_OBTER_TODOS",
                $"Erro ao obter todos os alunos: {ex.Message}", ex);
        }
    }   // ← fecha ObterTodos
    ...
    public static Aluno Map(DbDataReader reader, string nomeColumn = "nome")
    {
        try
        {
            ...
            // Aluno.Criar(int, string, string, DateOnly, string, string, Logradouro,
            //             string, string, string, Arquivo)
            // Argumentos posicionais na ordem exata da assinatura do método de fábrica.
        ...
        {
            throw new InfrastructureException("ERRO_MAPEAMENTO_ALUNO",
    ...
    public async Task<Aluno> Adicionar(
        Aluno entity, CancellationToken cancellationToken = default)
    {
        try
        {
        ...
        {
            throw new InfrastructureException("ERRO_ADICIONAR_ALUNO",
                $"Erro ao adicionar aluno: {ex.Message}", ex);
        }
    }   // ← fecha Adicionar
        ...
        Aluno entity, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_ATUALIZAR_ALUNO",
                $"Erro ao atualizar aluno ID {entity.Id}: {ex.Message}", ex);
        }
    }   // ← fecha Atualizar
    ...
    public async Task<bool> Remover(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_REMOVER_ALUNO",
                $"Erro ao remover aluno ID {id}: {ex.Message}", ex);
        }
    }   // ← fecha Remover
        ...
        Cpf cpf, CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException("ERRO_OBTER_POR_CPF",
                $"Erro ao obter aluno por CPF {cpf.Valor}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorCpf
        ...
        Email email, CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException("ERRO_OBTER_POR_EMAIL",
                $"Erro ao obter aluno por Email {email.Valor}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorEmail
        ...
        Cpf cpf, int? id = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "SELECT COUNT(1) FROM tb_aluno " +
                "WHERE cpf = @Cpf AND (@Id IS NULL OR id_aluno <> @Id)";
            await using var command = await CreateCommandAsync(query, cancellationToken);
            command.AddParameter("@Cpf", cpf.Valor, DbType.String);
        ...
        Email email, int? id = null, CancellationToken cancellationToken = default)
    {
        try
        {
            string query =
                "SELECT COUNT(1) FROM tb_aluno " +
                "WHERE email = @Email AND (@Id IS NULL OR id_aluno <> @Id)";
            await using var command = await CreateCommandAsync(query, cancellationToken);
            command.AddParameter("@Email", email.Valor, DbType.String);
            command.AddParameter("@Id", (object?)id ?? DBNull.Value, DbType.Int32);
            var count = Convert.ToInt32(await command.ExecuteScalarAsync(cancellationToken));
        ...
        string nome, CancellationToken cancellationToken = default)
    {
        try
        {
            string collate = _databaseType == DatabaseType.Sqlite ? " COLLATE NOCASE" : "";
            string query = $"{BaseSelectQuery} WHERE a.nome LIKE @Nome{collate} ORDER BY a.nome";
            await using var command = await CreateCommandAsync(query, cancellationToken);
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_NOME",
                $"Erro ao obter alunos por nome {nome}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorNome
        ...
        int id, Senha novaSenha, CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException("ERRO_TROCAR_SENHA",
                $"Erro ao trocar senha do aluno ID {id}: {ex.Message}", ex);
        }
    }   // ← fecha TrocarSenha

}   // ← fecha a classe AlunoRepository