...
""";
    ...
    public async Task<Matricula?> ObterMatriculaAtivaPorAluno(int alunoId, CancellationToken cancellationToken = default)
    {
        try
        {
            string query = $"{BaseSelectQuery} WHERE m.aluno_id = @AlunoId AND m.data_fim >= {DbProvider.GetCurrentDateFunction(_databaseType)} ORDER BY m.data_fim DESC";
            await using var command = await CreateCommandAsync(query, cancellationToken);
            command.AddParameter("@AlunoId", alunoId, DbType.Int32);
            await using var reader = await command.ExecuteReaderAsync(cancellationToken);
            return await reader.ReadAsync(cancellationToken) ? Map(reader) : null;
    ...
    public async Task<IEnumerable<Matricula>> ObterAtivas(int alunoId = 0, CancellationToken cancellationToken = default)
    {
        try
        {
            string query = $"{BaseSelectQuery} WHERE m.data_fim >= {DbProvider.GetCurrentDateFunction(_databaseType)} {(alunoId > 0 ? "AND m.aluno_id = @id" : "")} ORDER BY m.data_fim ASC";
            await using var command = await CreateCommandAsync(query, cancellationToken);
            if (alunoId > 0)
            {
    ...
    public async Task<IEnumerable<Matricula>> ObterVencendoEmDias(int dias, CancellationToken cancellationToken = default)
    {
        try
        {
            string currentDate = DbProvider.GetCurrentDateFunction(_databaseType);
            string maxDate = DbProvider.GetDateAddDaysExpression(currentDate, "@Dias", _databaseType);
