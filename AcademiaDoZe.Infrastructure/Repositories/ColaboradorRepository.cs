...
public class ColaboradorRepository : BaseRepository, IColaboradorRepository
{
        ...
        int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_ID",
                $"Erro ao obter colaborador por ID {id}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorId
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_TODOS",
                $"Erro ao obter todos os colaboradores: {ex.Message}", ex);
        }
    }   // ← fecha ObterTodos
    ...
    public static Colaborador Map(DbDataReader reader, string nomeColumn = "nome")
    {
        try
        {
            ...
            // Colaborador.Criar(int, string, string, DateOnly, string, string, Logradouro,
            //                   string, string, string, Arquivo, DateOnly, ColaboradorTipo, ColaboradorVinculo)
            // Usando argumentos posicionais na ordem exata da assinatura, já que os nomes
            // dos parâmetros não são "logradouro"/"endereco" como havíamos tentado antes.
        ...
        {
            throw new InfrastructureException("ERRO_MAPEAMENTO_COLABORADOR",
    ...
    public async Task<Colaborador> Adicionar(
        Colaborador entity, CancellationToken cancellationToken = default)
    {
        try
        {
            string query = DbProvider.FormatInsertQuery(
        ...
        {
            throw new InfrastructureException("ERRO_ADICIONAR_COLABORADOR",
                $"Erro ao adicionar colaborador: {ex.Message}", ex);
        }
    }   // ← fecha Adicionar
        ...
        Colaborador entity, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (rowsAffected == 0)
                throw new InfrastructureException("REGISTRO_NAO_ENCONTRADO",
        ...
        {
            throw new InfrastructureException("ERRO_ATUALIZAR_COLABORADOR",
                $"Erro ao atualizar colaborador ID {entity.Id}: {ex.Message}", ex);
        }
    }   // ← fecha Atualizar
    ...
    public async Task<bool> Remover(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_REMOVER_COLABORADOR",
                $"Erro ao remover colaborador ID {id}: {ex.Message}", ex);
        }
    }   // ← fecha Remover

    // ── ObterPorCpf ───────────────────────────────────────────────────────────
    public async Task<Colaborador?> ObterPorCpf(
        Cpf cpf, CancellationToken cancellationToken = default)
    {
        try
        {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_CPF",
                $"Erro ao obter colaborador por CPF {cpf.Valor}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorCpf
        ...
        Email email, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_EMAIL",
                $"Erro ao obter colaborador por Email {email.Valor}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorEmail

    // ── CpfJaExiste ───────────────────────────────────────────────────────────
    public async Task<bool> CpfJaExiste(
        Cpf cpf, int? id = null, CancellationToken cancellationToken = default)
    {
        try
        {
            string query =
        ...
        {
            throw new InfrastructureException("ERRO_VERIFICAR_CPF",
                $"Erro ao verificar existência de CPF: {ex.Message}", ex);
        }
    }   // ← fecha CpfJaExiste

    // ── EmailJaExiste ─────────────────────────────────────────────────────────
    public async Task<bool> EmailJaExiste(
        Email email, int? id = null, CancellationToken cancellationToken = default)
    {
        try
        {
            string query =
        ...
        {
            throw new InfrastructureException("ERRO_VERIFICAR_EMAIL",
                $"Erro ao verificar existência de Email: {ex.Message}", ex);
        }
    }   // ← fecha EmailJaExiste

    // ── ObterPorTipo ──────────────────────────────────────────────────────────
    public async Task<IEnumerable<Colaborador>> ObterPorTipo(
        ColaboradorTipo tipo, CancellationToken cancellationToken = default)
    {
        try
        {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_TIPO",
                $"Erro ao obter colaboradores por tipo {tipo}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorTipo

    // ── ObterPorVinculo ───────────────────────────────────────────────────────
    public async Task<IEnumerable<Colaborador>> ObterPorVinculo(
        ColaboradorVinculo vinculo, CancellationToken cancellationToken = default)
    {
        try
        {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_VINCULO",
                $"Erro ao obter colaboradores por vínculo {vinculo}: {ex.Message}", ex);
        }
    }   // ← fecha ObterPorVinculo

    // ── TrocarSenha ───────────────────────────────────────────────────────────
    public async Task<bool> TrocarSenha(
        int id, Senha novaSenha, CancellationToken cancellationToken = default)
    {
        try
        {
            string query =
        ...
        {
            throw new InfrastructureException("ERRO_TROCAR_SENHA",
                $"Erro ao trocar senha do colaborador ID {id}: {ex.Message}", ex);
        }
    }   // ← fecha TrocarSenha

}   // ← fecha a classe ColaboradorRepository