...
public class LogradouroInfrastructureTests : TestBase
{
    ...
    {
        var cep = GerarCep();
        var logradouroResult = Logradouro.Criar(0, cep, "Jonathan", "Souza Pereira", "SQLite", "SC", "Brasil");
    ...
    {
        var cep = GerarCep();
        var logradouro = Logradouro.Criar(0, cep, "Jonathan", "Souza Pereira", "SQLite", "SC", "Brasil").Value!;
        ...
        Assert.Equal("Jonathan", inserido.Nome);
    ...
    public async Task Logradouro_Atualizar_Sucesso()
    {
        ...
        var logradouroAtualizado = Logradouro.Criar(logradouro.Id, novoCep, "Jonathan", "Souza Pereira", "SQLServer", "SC", "Brasil").Value!;
        var resultado = await _repository.Atualizar(logradouroAtualizado);
        Assert.NotNull(resultado);
        Assert.Equal("Jonathan", resultado.Nome);
        Assert.Equal("Souza Pereira", resultado.Bairro);
        Assert.Equal("SQLServer", resultado.Cidade);
        var noBanco = await _repository.ObterPorId(logradouro.Id);
        Assert.NotNull(noBanco);
        Assert.Equal("Jonathan", noBanco.Nome);
    }
    [Fact]
